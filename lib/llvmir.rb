...
def create_context locals_hash={}
  {
    stack:[],
    locals:locals_hash,
    blocks:{}
...
def create_block name, context, &block
  context[:blocks][name] = ->{
    context[:stack].push name
    ret = block.call context[:stack], context[:locals], context[:blocks]
    context[:stack].pop
    ret
  }
end
...
def store val, locals, name, align=nil
  locals[name] = val
end

def load locals, name, align=nil
  locals[name]
end
...
def br cond_or_dest, true_label=nil, false_label=nil
  if true_label.nil?
    ...
    cond = cond_or_dest
    (cond ? true_label : false_label).call
  end
end
